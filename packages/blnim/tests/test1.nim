...
suite "round trip":
  test "scalars":
    for v in [nilValue(), nilValue(true), num"0", num"-123", text"",
              text"hi", sym"x", mark(sym"f"), bytes(@[byte 1, 2, 3])]:
      check roundTrips v
...
suite "rejections":
  ...
  test "sets and dicts must arrive sorted and unique":
    ...
    check rejects @[byte 0x80, 0x41, 0x62, 0x21, 0x31,
                    0x41, 0x61, 0x21, 0x32, 0xA0]             # {b:1 a:2}
    check rejects @[byte 0x80, 0x41, 0x61, 0x21, 0x31,
                    0x41, 0x61, 0x21, 0x32, 0xA0]             # {a:1 a:2}
...
suite "order is the encoding order":
  test "cmp(a, b) matches byte order of encodings":
    let vs = [nilValue(), num"0", num"9", num"10", num"-5", text"",
              text"a", text"b", text"aa", sym"a", mark(sym"a"),
              bytes(@[byte 0]), list(), list(sym"a"), list(sym"a", sym"b"),
              record(sym"h"), dict(@[(sym"k", num"1")]), set(num"1")]
    for a in vs:
      for b in vs:
...
suite "hashing":
  ...
  test "hash agrees with ==":
    let vs = [nilValue(), num"0", num"9", text"", text"a", sym"a",
              mark(sym"a"), bytes(@[byte 0]), list(sym"a"),
              record(sym"h"), dict(@[(sym"k", num"1")]), set(num"1")]
    for a in vs:
      for b in vs:
        if a == b:
