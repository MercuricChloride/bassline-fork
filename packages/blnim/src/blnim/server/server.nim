...
import ../codec/[stream, encode]
export asyncnet, asyncdispatch, stream, encode
...
proc connect*(host: string; port: Port; maxDepth = 64;
              maxValueBytes = DefaultMaxValueBytes): Future[Conn] {.async.} =
...
proc landing*(port: Port; onValue: Handler;
  ...
  try:
    ...
    socket.close()  # asyncnet has no finalizer; the fd would leak
    raise
  Landing(socket: socket, onValue: onValue, onOpen: onOpen,
          onClose: onClose, onError: onError,
          maxDepth: maxDepth, maxValueBytes: maxValueBytes)
...
proc serve*(l: Landing) {.async.} =
  ...
  while not l.socket.isClosed:
    ...
    try:
      ...
      continue
    let conn = Conn(socket: accepted.client, address: accepted.address,
