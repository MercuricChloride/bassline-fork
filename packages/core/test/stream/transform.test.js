...
describe('encodeStream / decodeStream (node Transform)', () => {
  ...
  it('emits error on a malformed header byte', async () => {
    ...
    expect(String(await error)).toMatch(/invalid tag/)
  })

  it('errors when the stream ends mid-value', async () => {
    const enc = encode(text('truncated'))
    const ds = decodeStream()
    ds.resume()
    const error = new Promise(res => ds.on('error', res))
  ...
  it('honours maxValueBytes from the value header alone', async () => {
    const enc = encode(bytes(new Uint8Array(100))) // header 0x57 0x64 + 100 bytes
    const ds = decodeStream({ maxValueBytes: 8 })
    ds.resume()
    const error = new Promise(res => ds.on('error', res))
    ds.end(Buffer.from(enc.subarray(0, 2)))
    expect(String(await error)).toMatch(/value-size limit/)
  })
})
