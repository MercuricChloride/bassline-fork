...
import { Decoder } from '../codec/decode.js'
import { valueFromView } from '../value/view.js'
 ...
 *
 * A malformed or over-large value emits `'error'`; a stream ending mid-value
 * errors on flush.
 */
export class DecodeStream extends Transform {
  /**
   * @param {{ maxValueBytes?: number, maxDepth?: number } &
   *   import('node:stream').TransformOptions} [options]
   */
  constructor({ maxValueBytes, maxDepth, ...transformOptions } = {}) {
    super({ ...transformOptions, readableObjectMode: true })
    this._dec = new Decoder({ maxValueBytes, maxDepth })
  }
  ...
  _transform(chunk, _encoding, callback) {
    try {
      for (const view of this._dec.push(chunk)) this.push(valueFromView(view))
      callback()
    } catch (err) {
  ...
  /** @param {(error?: Error | null) => void} callback */
  _flush(callback) {
    callback(this._dec.pending ? new Error('stream ended mid-value') : null)
  }
}
