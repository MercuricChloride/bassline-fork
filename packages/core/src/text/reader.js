// @ts-check
// Reader for the Bassline textual syntax. Values are built through the same
// constructors the rest of the system uses, so a read value is the same kind
// of thing as one built any other way.

/** @import { Value } from '../value/value.js' */
import { CANONICAL_INT } from '../codec/payload.js'
import { eq, withMark } from '../value/value.js'
import {
  bytes,
  dict,
  int,
...
/**
 * A read that ran out of input mid-value — a prefix of something that could
 * still be valid, not a syntax error. A subclass of {@link ReaderError}, so a
 * `catch (ReaderError)` still catches it.
 */
export class ReaderIncomplete extends ReaderError {}
 ...
 * @property {Value} value the parsed value
 * @property {number} start offset of its first source character
 * @property {number} end offset just past its last source character
 * @property {Spanned[]} children sub-spans in document order (none for an atom)
 */
...
export function readSpans(source, opts) {
  ...
  /**
   * The input ended mid-value; feeding more could still make it valid.
   * @param {number} p
   * @param {string} msg
   * @returns {never}
   */
  function incomplete(p, msg) {
    throw new ReaderIncomplete(source, p, msg)
  }
  ...
  function readQuoted(closer, what) {
    let out = ''
    while (true) {
      if (pos >= n) incomplete(pos, `unterminated ${what}`)
      const c = source[pos]
      if (c === closer) {
        pos++
        return out
  ...
  function readUntil(opener, closer) {
    ...
    while (true) {
      skipTrivia()
      if (pos >= n) incomplete(pos, `unclosed ${opener}`)
      if (source[pos] === closer) {
        pos++
        return items
      }
      items.push(readSpanned())
    }
  }
  ...
  function readBraces() {
    skipTrivia()
    if (pos >= n) incomplete(pos, 'unclosed {')
    if (source[pos] === '}') {
      pos++
  ...
  function readDictAfter(firstKey) {
    ...
    while (true) {
      ...
      key = readSpanned()
      skipTrivia()
      if (pos >= n) incomplete(pos, "dict entry needs ':' after its key")
      if (source[pos] !== ':') fail(pos, "dict entry needs ':' after its key")
      pos++
    }
  ...
  function readSetAfter(first) {
    ...
    if (value.size !== items.length) failDuplicate(items, 'set member')
    return { value, children: items }
  }

  /**
   * Construct a text/symbol, converting a well-formedness rejection into a
  ...
  function readSpanned() {
    ...
    if (source[pos] === '!') {
      ...
      pos++
      if (isEOF(source[pos])) incomplete(pos, 'mark with no value')
    ...
    const c = source[pos]
    if (isEOF(c)) incomplete(pos, 'expected a value')
    ...
    if (c === '[') {
      ...
      pos++
      value = wellFormed(text, readQuoted('"', 'string'), start, 'string')
    } else if (c === "'") {
      pos++
